...
export default class Content extends Component {
  ...
  async componentDidMount() {
    ...
    const mainData = await GetData(
      `http://localhost:3000/user/${this.props.id}`,
      this.props.id
    );
    const activity = await GetData(
      `http://localhost:3000/user/${this.props.id}/activity`,
      this.props.id
    );
    const averageSessions = await GetData(
      `http://localhost:3000/user/${this.props.id}/average-sessions`,
      this.props.id
    );
    const performance = await GetData(
      `http://localhost:3000/user/${this.props.id}/performance`,
      this.props.id
    );
          ...
          <div className="content-wrapper">
            <div className="graphs-container">
              ...
              <div className="sub-graphs-container">
                <AverageSession
                  averageSessions={this.state.userAverageSessions.sessions}
                />
                <Performance performance={this.state.userPerformance} />
                <Score
                  score={this.state.userMainData?.score}
                  todayScore={this.state.userMainData?.todayScore}
                />
              </div>
            </div>
